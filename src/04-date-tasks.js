...
function timeSpanToString(startDate, endDate) {
  let sub = endDate - startDate;
  let result = `${(sub % 1000).toString().padStart(3, '0')}`;

  sub = Math.floor(sub / 1000);
  result = `${(sub % 60).toString().padStart(2, '0')}.${result}`;
...
function angleBetweenClockHands(date) {
  ...
  const hoursDegree = h * 30 + m / 2;
  let degree = Math.abs(hoursDegree - m * 6);
